import { Upload, LayoutDashboard, AlertCircle, FileText, Settings, ChevronRight, ChevronLeft, Shield, Sun, Moon, ScanLine, ScanBarcode, Sparkles, BarChart3, Building2, Database, TrendingUp, Receipt, Truck, Users, Layers, AlertTriangle, X } from "lucide-react";
...
export function Sidebar({ activeTab, onTabChange, expanded, onToggle, theme, onThemeToggle, mobileOpen = false, onClose }: SidebarProps) {
  const isDark = theme === "dark";
  // On mobile the drawer is always full width with labels; on desktop, labels follow the expand state.
  ...
  return (
    <aside
      className={[
        "flex flex-col bg-white border-r border-gray-200 py-4 z-30 transition-all duration-200 ease-in-out",
        "fixed inset-y-0 left-0 md:static md:shrink-0 shadow-2xl md:shadow-none",
        "w-64", expanded ? "md:w-[220px]" : "md:w-16",
        mobileOpen ? "translate-x-0" : "-translate-x-full",
        "md:translate-x-0",
      ].join(" ")}
    >
      {/* Logo + toggle */}
      <div className={`flex items-center mb-6 px-3 ${showLabels ? "justify-between" : "justify-center"}`}>
        ...
        <button
          onClick={onToggle}
          className="hidden md:flex w-6 h-6 rounded-md items-center justify-center text-gray-400 hover:text-violet-600 hover:bg-violet-50 transition-colors"
        ...
        <button
          onClick={onClose}
          className="md:hidden w-7 h-7 rounded-md flex items-center justify-center text-gray-400 hover:text-violet-600 hover:bg-violet-50 transition-colors"
      ...
      {/* Theme toggle */}
      <div className={`px-2 mb-2 ${showLabels ? "" : "flex justify-center"}`}>
        <button
          onClick={onThemeToggle}
          title={isDark ? "Switch to light mode" : "Switch to dark mode"}
          className={[
            "flex items-center gap-3 rounded-xl px-2.5 h-10 transition-colors",
            showLabels ? "w-full" : "justify-center w-10",
            "text-gray-500 hover:bg-violet-50 hover:text-violet-700",
          ].join(" ")}
        >
      ...
      {/* User avatar */}
      <div className={`px-2 ${showLabels ? "" : "flex justify-center"}`}>
        <div className={`flex items-center gap-2.5 rounded-xl px-2.5 py-2 ${showLabels ? "" : "justify-center"}`}>
